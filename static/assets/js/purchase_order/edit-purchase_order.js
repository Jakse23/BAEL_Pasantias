$(document).on('click', '.btn-edit-order', function () {
    ...
    $.get('/purchase_order/edit_purchase_order/' + orderId + '/', function (data) {
        // Renderiza el formulario con los datos recibidos
        let order = data.order;
        let details = data.details;
        // Opciones (puedes optimizar esto con templates JS)
        let supplierOptions = $('select[name="fk_supplier"]').html();
        let currencyOptions = $('select[name="currency"]').html();
        let paymentOptions = $('select[name="payment_type"]').html();
        let shippingOptions = $('select[name="shipping_type"]').html();
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <select name="fk_supplier" class="form-control mb-3">${supplierOptions}</select>
                <label>Moneda</label>
                <select name="currency" class="form-control mb-3">${currencyOptions}</select>
                <label>Tipo de Pago</label>
                <select name="payment_type" class="form-control mb-3">${paymentOptions}</select>
                <label>Tipo de Envío</label>
                <select name="shipping_type" class="form-control mb-3">${shippingOptions}</select>
            </div>
            <div class="col-md-6">
                <label>Fecha de Inicio</label>
                <input type="datetime-local" name="start_date" class="form-control mb-3" required>
                <label>Fecha Estimada de Llegada</label>
                <input type="datetime-local" name="estimated_arrival" class="form-control mb-3" required>
                <label>Fecha de Fin</label>
                <input type="datetime-local" name="end_date" class="form-control mb-3" required>
                <label>Observación</label>
                <input type="text" name="observation" class="form-control mb-3">
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                <label>Ubicación de Destino</label>
                <select name="fk_destination_location" class="form-control mb-3">${locationOptions}</select>
            </div>
        </div>
        <hr>
        ...
        f.find('[name="fk_supplier"]').val(order.fk_supplier);
        f.find('[name="currency"]').val(order.currency);
        f.find('[name="payment_type"]').val(order.payment_type);
        f.find('[name="shipping_type"]').val(order.shipping_type);
            ...
            $list.append(`
                <div class="dynamic-detail row mb-2">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" step="0.01" name="unit_price[]" class="form-control numeric-only" required value="${d.unit_price}">
                    </div>
                    <div class="col-md-3">
    ...
    $('#edit-order-form').off('submit').on('submit', function (e) {
        ...
        // Calcular el total
        let total = 0;
        $('#edit-order-form input[name="unit_price[]"]').each(function () {
        ...
        Swal.fire({
            title: "¿Estás seguro?",
            text: "¿Deseas guardar los cambios en esta orden de compra?",
            icon: "question",
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/purchase_order/edit_purchase_order/' + orderId + '/',
                    method: 'POST',
                    data: $.param(formData),
